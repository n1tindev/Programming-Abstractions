...
public class StreamUtils {
    ...
    public static String longest(Collection<String> strings, boolean from_start) {
        return (from_start) ? (strings.stream().reduce((s1, s2) -> s1.length() >= s2.length() ?
                s1 : s2).orElse(null)) : (strings.stream().reduce((s2, s1) -> s2.length() > s1.length() ?
                s2 : s1).orElse(null));
    }
    ...
    public static <T extends Comparable<T>> T least(Collection<T> items, boolean from_start) {
        return (from_start) ? (items.stream().reduce((s1, s2) -> s1.compareTo(s2) >= 0 ?
                s2 : s1).orElse(null)) : (items.stream().reduce((s1, s2) -> s1.compareTo(s2) > 0 ?
                s2 : s1).orElse(null));
    }
